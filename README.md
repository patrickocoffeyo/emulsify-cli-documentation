...
- **componentCategories** (REQUIRED if no `repository` field is present): Array containing objects that describe each category of components. Each object MUST provide a `name`, SHOULD provide a `description`, and MUST have a `path` property who's value is a relative path to the folder containing the components within the category.
